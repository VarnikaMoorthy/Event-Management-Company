...
if (isset($_POST['update'])) {
  ...
  $remark = $_POST['remark'];
  $ordid = $_GET['oid'];
  $myqry=mysqli_query($db,"select frm_id from remark where frm_id='$form_id'");
  
  if(mysqli_num_rows($myqry)>0)
  {
    $query=mysqli_query($db,"update remark set status='$status',remark='$remark' where o_id='$ordid'");
    echo "<script>alert('Updated successfully');</script>";
  }
  else
  {
  $query = mysqli_query($db, "insert into remark(frm_id,o_id, status,remark) values('$form_id',$ordid, '$status','$remark')");
